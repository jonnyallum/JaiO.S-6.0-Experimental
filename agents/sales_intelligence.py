...
class SalesIntelligenceState(BaseState):
    workflow_id:   str
    timestamp:     str
    agent:         str
    error:         str | None
...
def build_graph():
    ...
    g = StateGraph(SalesIntelligenceState)
    g.add_node("sales_intelligence", sales_intelligence_node)
    g.set_entry_point("sales_intelligence")
    g.add_edge("sales_intelligence", END)
    return g.compile()
