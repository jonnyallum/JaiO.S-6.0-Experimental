...
class DevOpsEngineerState(BaseState):
    workflow_id:    str
    timestamp:      str
    agent:          str
    error:          str | None
...
def build_graph():
    ...
    g = StateGraph(DevOpsEngineerState)
    g.add_node("devops_engineer", devops_engineer_node)
    g.set_entry_point("devops_engineer")
    g.add_edge("devops_engineer", END)
    return g.compile()
