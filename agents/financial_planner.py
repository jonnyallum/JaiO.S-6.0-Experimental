...
class FinancialPlannerState(BaseState):
    workflow_id:   str
    timestamp:     str
    agent:         str
    error:         str | None
...
def build_graph():
    ...
    g = StateGraph(FinancialPlannerState)
    g.add_node("financial_planner", financial_planner_node)
    g.set_entry_point("financial_planner")
    g.add_edge("financial_planner", END)
    return g.compile()
